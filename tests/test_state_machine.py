...
no permita transiciones inválidas.
"""
...
class TestApplyAction:
    ...
    def test_approve_area_pasa_directo_a_quotation(self):
        """Sin RN-MONTO: APPROVE_AREA va directo a PENDING_QUOTATION."""
...
class TestAvailableActions:
    ...
    def test_pending_area_approval_tiene_approve_y_reject(self):
        actions = available_actions(SCStatus.PENDING_AREA_APPROVAL)
        assert SCAction.APPROVE_AREA in actions
        assert SCAction.REJECT_AREA in actions
        assert SCAction.CANCEL in actions
...
class TestFlujoCompletoFelizPath:
    """Flujo completo simplificado (sin estados de monto/budget): DRAFT → CLOSED."""
